...
CELLS: list[tuple[str, str]] = [
    (
        "md",
        r"""# pyinfluence: a tour of training-data attribution
...
""",
    ...
    (
        "md",
        r"""## 1. The scenario: a regression dataset with a known label bug
...
""",
    ...
    (
        "code",
        r"""attr = InfluenceFunctions(mode='loss', damping=1e-3).fit(model, X_train, y_train)
...
""",
    ...
    (
        "md",
        r"""## 3. Find the corrupted rows: `plot_self_influence`
...
2·std).""",
    ...
    (
        "code",
        r"""self_inf = self_influence(attr)
...
""",
    ...
    (
        "md",
        r"""When per-sample training error is also available, the *scatter* form is
...
much stronger candidate than one with high self-influence alone.""",
    ...
    (
        "code",
        r"""train_err = np.abs(model.predict(X_train) - y_train)
...
""",
    ...
    (
        "md",
        r"""Because we *know* the corrupted rows here, we can validate the ranking
...
different trace entirely. Inject the kind of errors you actually expect.""",
    ...
    (
        "code",
        r"""is_corrupted = np.zeros(n_train, dtype=bool)
...
""",
    ...
    (
        "md",
        r"""## 4. Explain one prediction: `plot_top_influencers`
...
badly:""",
    ...
    (
        "code",
        r"""test_err = np.abs(model.predict(X_test) - y_test)
...
""",
    ...
    (
        "md",
        r"""## 5. Are the scores meaningful? `plot_removal_curve`
...
""",
    ...
    (
        "md",
        r"""Inverting the direction, removing the *helpful* tail first, should hurt
the model dramatically. It does:""",
    ),
    (
        "code",
        r"""curve_p = removal_curve(
    attr, X_test, y_test,
    fractions=np.linspace(0.0, 0.3, 7),
    direction='helpful',
    n_random=5,
...
""",
    ...
    (
        "md",
        r"""## 6. Audit a subgroup: `plot_by_group`
...
aggregate confirms it.""",
    ...
    (
        "code",
        r"""fig, _ = viz.plot_by_group(scores, sources, style='bar', method='sum')
plt.show()
""",
    ),
    (
        "md",
        r"""The bar chart collapses each group to a single number. To inspect the
...
""",
    ...
    (
        "md",
        r"""## 7. Do methods agree? `plot_method_comparison`
...
""",
    ...
    (
        "md",
        r"""## 8. Signal or noise? Error bars on Monte-Carlo estimators
...
should not over-interpret.""",
    ...
    (
        "code",
        r"""attr_bs = BootstrapInfluence(mode='loss', n_estimators=80, random_state=0, verbose=0)
...
""",
    ...
    (
        "md",
        r"""## 9. Is the ranking stable? `plot_top_k_stability`
...
"important point" than one that only sneaks in once.""",
    ...
    (
        "code",
        r"""reps = stability_replicates(attr, X_test[:15], y_test[:15],
...
""",
    ...
    (
        "md",
        r"""## 10. Cross-sample structure: `plot_heatmap`
...
mass so it remains readable as `n_train` grows.""",
    ...
    (
        "code",
        r"""fig, _ = viz.plot_heatmap(scores, top_k=20)
plt.show()
""",
    ),
    (
        "md",
        r"""## 11. How concentrated is the influence? `plot_influence_concentration`
...
rows make the curve hug the top-left.""",
    ...
    (
        "code",
        r"""fig, _ = viz.plot_influence_concentration(scores)
plt.show()
""",
    ),
    (
        "md",
        r"""## 12. Working with named samples
...
`Index`, or a pandas `Series` (the same idiom: `labels=df.index`).""",
    ...
    (
        "code",
        r"""# Build readable labels of the form 'src_C_017' (source + row index).
...
""",
    ...
    (
        "md",
        r"""## 13. One-liner overview: `report`
...
""",
    ...
    (
        "md",
        r"""## 14. Real data: breast-cancer classification
...
""",
    ...
    (
        "md",
        r"""## 15. Functional influence: attribute *any* scalar property of the model
...
per-test-point loss influence.""",
    ...
    (
        "code",
        r"""from pyinfluence import FunctionalInfluence, functionals
...
""",
    ...
    (
        "md",
        r"""## 16. Fairness: which training points drive a disparity?
...
one-liner.""",
    ...
    (
        "code",
        r"""from pyinfluence.fairness import disparity, disparity_removal_curve
