...
differences. The fairness vocabulary (demographic parity, equal
opportunity, ...) is a thin naming layer over these in
:mod:`pyinfluence.fairness`.
...
def mean(of: ValueKind = "scores") -> Functional:
    ...
    def grad(v, y=None):
        return np.full(np.asarray(v).size, 1.0 / np.asarray(v).size)

    return Functional(fn=fn, grad=grad, of=of, name=f"mean_{of}")
...
def group_gap(
    ...
    keep : callable, optional
        ...
        ``lambda y: y == pos`` for a TPR-style gap). Requires ``y`` at
        evaluation time.
    of : {'scores', 'losses'}, default='scores'
        Value kind the gap is computed over.
    """
    mask_g1 = _two_groups(groups)
    ...
    def _masks(y):
        ...
        k = np.asarray(keep(np.asarray(y).ravel()), dtype=bool)
        m1 = mask_g1[k]
    ...
    return Functional(fn=fn, grad=grad, of=of, name="group_gap")
...
def cohens_d(groups: ArrayLike) -> Functional:
    ...
    def fn(v, y=None):
        s1, s0, n1, n0, pooled_var = _stats(v)
        return float((s1.mean() - s0.mean()) / np.sqrt(pooled_var))

    def grad(v, y=None):
        s1, s0, n1, n0, sp2 = _stats(v)
        sp = np.sqrt(sp2)
        d = (s1.mean() - s0.mean()) / sp
        g = np.empty(np.asarray(v).size)
        # d(mean gap)/dv +- 1/(n_g sp), minus the pooled-SD term
        g[mask] = (1.0 / n1) / sp - d * (s1 - s1.mean()) / ((n1 + n0 - 2) * sp2)
        g[~mask] = (-1.0 / n0) / sp - d * (s0 - s0.mean()) / ((n1 + n0 - 2) * sp2)
        return g

    return Functional(fn=fn, grad=grad, of="scores", name="cohens_d")
...
def auroc(pos_label, tau: float | None = None) -> Functional:
    ...
    def _split(v, y):
        ...
        v = np.asarray(v, dtype=float).ravel()
        pos = np.asarray(y).ravel() == pos_label
    ...
    if tau is None:
        ...
        return Functional(fn=fn, grad=grad, of="scores", name="auroc")

    def fn(v, y=None):
        v, pos = _split(v, y)
        diff = (v[pos][:, None] - v[~pos][None, :]) / tau
        return float(_sigmoid(diff).mean())

    def grad(v, y=None):
        v, pos = _split(v, y)
        n_pos, n_neg = int(pos.sum()), int((~pos).sum())
        s = _sigmoid((v[pos][:, None] - v[~pos][None, :]) / tau)
        w = s * (1.0 - s) / (n_pos * n_neg * tau)
        g = np.zeros(v.size)
        g[pos] = w.sum(axis=1)
        g[~pos] = -w.sum(axis=0)
        return g

    return Functional(fn=fn, grad=grad, of="scores", name=f"auroc_tau={tau:g}")
...
def worst_group_mean(
    ...
    """
    g = np.asarray(groups).ravel()
    group_masks = [g == u for u in np.unique(g)]

    def fn(v, y=None):
        return float(max(v[m].mean() for m in group_masks))

    def grad(v, y=None):
        means = [v[m].mean() for m in group_masks]
        m = group_masks[int(np.argmax(means))]
        out = np.zeros(np.asarray(v).size)
        out[m] = 1.0 / m.sum()
        return out

    return Functional(fn=fn, grad=grad, of=of, name="worst_group_mean")
